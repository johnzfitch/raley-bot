...
def save_result_to_file(filename_prefix: str, data: dict) -> str:
    """Save result data atomically with restricted permissions."""
    ...
    # Write to a temporary file with owner-only permissions, then atomically rename.
    fd, temp_path = tempfile.mkstemp(prefix=f".{filename_prefix}-", suffix=".tmp", dir=base_dir)
    try:
        os.fchmod(fd, 0o600)
...
async def handle_search(args: dict) -> str:
    ...
    decision = evaluate_options(products_as_dicts, query, prefer_organic=_prefs.general.prefer_organic)
...
async def handle_build_list(args: dict) -> str:
    ...
    for item_name, qty in parsed:
        ...
        decision = evaluate_options(products, item_name, prefer_organic=_prefs.general.prefer_organic)
