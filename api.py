...
_COOKIE_UNSAFE_PATTERN = re.compile(
    r"[\x00-\x1f\x7f]|%0[da]",
    re.IGNORECASE,
)
...
def _is_safe_cookie_component(name: str, value: str) -> bool:
    """Return False if name/value contains control chars or encoded CRLF."""
    if _COOKIE_UNSAFE_PATTERN.search(name) or _COOKIE_UNSAFE_PATTERN.search(value):
        return False
...
def _build_cookie_header(cookies: list[dict]) -> str | None:
    """Build cookie header, dropping unsafe cookie pairs entirely."""
    safe_pairs = []
    for cookie in cookies:
        name = str(cookie.get("name", ""))
        value = str(cookie.get("value", ""))
        if not name or not value or len(value) >= MAX_COOKIE_VALUE_LEN:
            continue
        if not _is_safe_cookie_component(name, value):
            continue
...
def _is_valid_header_name(name: str) -> bool:
    """Validate header name is a valid HTTP token (RFC 7230)."""
    return bool(name and _HEADER_TOKEN_RE.match(name))


def _run_curl_subprocess(cmd: list[str], timeout: int = DEFAULT_TIMEOUT) -> tuple[str, int]:
    """Run curl with process-group isolation for clean timeout handling."""
    proc = Popen(
        cmd,
        stdout=PIPE,
        stderr=PIPE,
...
class CurlClient:
    ...
    def __init__(self, cookies: list[dict]):
        self.cookie_str = _build_cookie_header(cookies)
    ...
    def _run_curl(
        ...
        # Ensure attacker-controlled URL cannot be treated as a curl option
        cmd.extend(["--", url])

        try:
            output, _ = _run_curl_subprocess(cmd, timeout=DEFAULT_TIMEOUT)
        except TimeoutExpired:
...
def clip_all_offers(client: CurlClient) -> tuple[int, int, list[str]]:
